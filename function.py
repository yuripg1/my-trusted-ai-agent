...
def get_formatted_bash_command_output(command: str, permission_granted: bool, stdout: str, stderr: str, returncode: int) -> str:
    if not permission_granted:
...
def search_web(query: str, max_results: int, page: int) -> str:
    ...
    for raw_search_result in raw_search_results:
        search_results.append(DuckDuckGoSearchResult(title=raw_search_result["title"],href=raw_search_result["href"],body=raw_search_result["body"]))
    text_results: list[str] = []
    for search_result in search_results:
        text_results.append(f"<search_result>\n<title>{search_result["title"]}</title>\n<href>{search_result["href"]}</href>\n<body>\n{search_result["body"]}\n</body>\n</search_result>")
