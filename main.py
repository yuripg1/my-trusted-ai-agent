...
from function import execute_function_call, execute_bash_command, get_formatted_bash_command_output, get_function_call_message, get_default_function_call_permission
            ...
            while True:
                ...
                if user_input == "/rewind":
                    ...
                    if len(deepseek_messages) == 0:
                        ai.initialize_messages(system_messages=get_system_messages(environment,ui.get_system_instruction()), deepseek_messages=deepseek_messages)
                    ai.add_to_messages(deepseek_messages=deepseek_messages, deepseek_role="user",deepseek_content=user_input)
                    while True:
                        ai.request_reply(deepseek_messages=deepseek_messages)
                        assistant_message = deepseek_messages[-1]
                        ui.display_assistant_message(0, assistant_message["content"], assistant_message["reasoning_content"])
                        ...
                        for deepseek_tool_call in deepseek_tool_calls:
                            ...
                            if function_call is not None:
                                ...
                                if len(tool_message) != 0:
                                    ai.add_to_messages(deepseek_messages, "tool", tool_message, "", [], function_call["info"]["tool_call_id"])
